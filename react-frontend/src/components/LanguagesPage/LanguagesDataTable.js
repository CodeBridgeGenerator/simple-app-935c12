...
const LanguagesDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const dropdownArrayTemplate1 = (rowData, { rowIndex }) => <p >{rowData.types}</p>
const countries = JSON.parse('{"0":"Angeric","1":"Badwin","2":"Charlo"}');
const dropdownObjectTemplate2 = (rowData, { rowIndex }) => <p >{countries[rowData.countries]}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "languages"});
...
<Column field="types" header="Types" body={dropdownArrayTemplate1} filter={selectedFilterFields.includes("types")} hidden={selectedHideFields?.includes("types")}  style={{ minWidth: "8rem" }} />
<Column field="countries" header="Countries" body={dropdownObjectTemplate2} filter={selectedFilterFields.includes("countries")} hidden={selectedHideFields?.includes("countries")}  style={{ minWidth: "8rem" }} />
        ...
        <Dialog header="Upload Languages Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService />
      </Dialog>

      <Dialog header="Search Languages" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
