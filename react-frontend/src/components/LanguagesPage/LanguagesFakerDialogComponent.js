...
import languagesFakerFactory from "./languagesFakerFactory";
...
const LanguagesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = languagesFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("languages").create(elem));
...
export default connect(mapState, mapDispatch)(LanguagesFakerDialogComponent);
