...
import LanguagesEditDialogComponent from "./LanguagesEditDialogComponent";
import LanguagesCreateDialogComponent from "./LanguagesCreateDialogComponent";
import LanguagesFakerDialogComponent from "./LanguagesFakerDialogComponent";
import LanguagesSeederDialogComponent from "./LanguagesSeederDialogComponent";
...
const LanguagesPage = (props) => {
    ...
    useEffect(() => {
        ...
        client
            ...
            .catch((error) => {
                console.log({ error });
                setLoading(false);
                props.alert({ title: "Languages", type: "error", message: error.message || "Failed get Languages" });
            });

    const service = _.find(config.services, { serviceName : "languages"});
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("languages").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Languages", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
        ...
        const promises = data.map((e) => client.service("languages").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
        ...
        <div className="mt-5">
            <div className="grid">
                <div className="col-6 flex justify-content-start">
                    <h3 className="mb-0 ml-2">Languages </h3>
                </div>
                <div className="col-6 flex justify-content-end">
                    <>
                    <Button label="add" loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="languages-add-button"/>
            ...
            <div className="grid align-items-center">
                <div className="col-12" role="languages-datatable">
                    <LanguagesDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>
            ...
            <LanguagesEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <LanguagesCreateDialogComponent entity={newRecord} show={showCreateDialog} onHide={() => setShowCreateDialog(false)} onCreateResult={onCreateResult} />
            <LanguagesFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <LanguagesSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(LanguagesPage);
